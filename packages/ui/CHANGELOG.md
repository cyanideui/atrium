...
- **Subpath entries** — `@atrium/ui/command-palette` (cmdk) and `@atrium/ui/date-picker` (react-day-picker) for bundle-size-conscious consumers.
...
### Breaking changes
- Package renamed from `@erp-ds/ui` to `@atrium/ui`. Update imports.
...
1. Update `package.json`:
   ...
   + "@atrium/ui": "^1.0.0"
   ```
2. Find-replace `from "@erp-ds/ui"` → `from "@atrium/ui"`.
