...
import { Button, Icon, Progress } from "@cyanideui/ui"
import { File01Icon, Image01Icon, Pdf01Icon, CheckmarkCircle02Icon, Delete02Icon } from "@hugeicons/core-free-icons"
import type { IconSvgElement } from "@hugeicons/react"
import { PageHeader, Section } from "../../components/page-shell"
import { CodeBlock } from "../../components/code-block"

type FileKind = "image" | "pdf" | "other"
interface FileItem { id: string; name: string; size: string; kind: FileKind; progress?: number }
const KIND_ICON: Record<FileKind, IconSvgElement> = { image: Image01Icon, pdf: Pdf01Icon, other: File01Icon }
  ...
  return (
    <>
      <PageHeader
        ...
        title="File list"
        status="stable"
        description="List of uploaded/attached files — type icon, name, size, an upload-progress bar (while uploading) or a done check (when complete), plus a remove button. Pairs with FileUpload."
      />
      <Section title="Preview" description="Remove a file to see the list update.">
            ...
            {files.map((f) => {
              ...
              return (
                <li key={f.id} className="flex items-center gap-3 px-3 py-2.5">
                  ...
                  <Button variant="tertiary" size="sm" aria-label={`Remove ${f.name}`} onClick={() => remove(f.id)}>
      ...
      <Section title="Install"><CodeBlock language="bash" code={`npx cyanideui add file-list`} /></Section>
    </>
  )
}
