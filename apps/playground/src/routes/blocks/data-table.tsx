...
export function DataTableBlock() {
  ...
  const filtered = React.useMemo(() => {
    ...
    if (query) {
      ...
      rows = rows.filter((r) => r.name.toLowerCase().includes(q) || r.email.toLowerCase().includes(q) || r.id.includes(q))
    }
    return rows
  }, [query, view])
  ...
  const togglePage = () => {
    if (allOnPage) setSelected((s) => s.filter((id) => !visible.some((r) => r.id === id)))
    else setSelected((s) => Array.from(new Set([...s, ...visible.map((r) => r.id)])))
  }
  ...
  return (
    <>
      <PageHeader
        ...
        title="Data table"
        status="stable"
        description="Complete table experience — search + saved-view presets, row selection with a bulk-actions bar, empty state, and pagination. Client-side logic ready to wire to a query."
      />

      <Section title="Preview" description="Select rows to reveal the bulk-actions bar; search and switch saved views to filter.">
                ...
                {visible.length === 0 ? (
                  ...
                  visible.map((row) => (
                    <TableRow key={row.id} selected={selected.includes(row.id)}>
                      ...
                      <TableCell><Badge tone={row.status}>{row.statusLabel}</Badge></TableCell>
      ...
      <Section title="Install" description="Pulls the block plus its full component dependency graph (table, search, saved filters, bulk actions, pagination).">
        <CodeBlock language="bash" code={`npx cyanideui add data-table`} />
      </Section>
    </>
  )
}
