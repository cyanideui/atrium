import { Icon, Progress, cn } from "@cyanideui/ui"
import { ArrowUpRight01Icon, ArrowDown01Icon } from "@hugeicons/core-free-icons"
import { PageHeader, Section } from "../../components/page-shell"
import { CodeBlock } from "../../components/code-block"

interface Metric { label: string; current: number; previous: number; format?: (n: number) => string; goodWhenUp?: boolean }
...
function pct(c: number, p: number) { if (p === 0) return c === 0 ? 0 : 100; return ((c - p) / p) * 100 }

export function MetricComparisonBlock() {
  ...
  return (
    <>
      <PageHeader
        ...
        status="stable"
        description="This-period vs last-period metrics — current value, prior value, signed delta, and a bar scaled to the group max. goodWhenUp flips the delta color for metrics where down is better."
      />
                ...
                return (
                  <li key={m.label} className="flex flex-col gap-1.5 px-4 py-3">
                    <div className="flex items-baseline justify-between gap-3">
                      ...
                      <span className={cn("inline-flex items-center gap-0.5 text-[12px] font-medium tabular-nums", good ? "text-success" : "text-error")}>
                        <Icon icon={up ? ArrowUpRight01Icon : ArrowDown01Icon} size={12} aria-hidden />
                        {up ? "+" : ""}{delta.toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex items-baseline justify-between gap-3">
                      <span className="text-[18px] font-semibold tabular-nums tracking-tight text-ink">{fmt(m.current)}</span>
      ...
      <Section title="Install"><CodeBlock language="bash" code={`npx cyanideui add metric-comparison`} /></Section>
    </>
  )
}
