  ...
  return (
    <>
      <PageHeader
        ...
        status="stable"
        description="Line-item table with a subtotal / tax / total breakdown and a footer CTA. Totals compute from the items. Use for invoices, order summaries, checkout review."
      />
      <Section title="Preview">
        <Card className="w-full max-w-[520px]">
          <CardHeader>
            ...
            <CardAction>
              <Button variant="secondary" size="sm" leading={<Icon icon={Download04Icon} size="sm" />}>PDF</Button>
            </CardAction>
          </CardHeader>
          <Table fixed>
            <TableHeader>
          ...
          <div className="flex flex-col gap-1.5 border-t border-hairline px-4 py-3 text-[13px]">
            <div className="flex justify-between text-ink-2"><span>Subtotal</span><span className="tabular-nums">{money(subtotal)}</span></div>
            <div className="flex justify-between text-ink-2"><span>Tax ({(TAX_RATE * 100).toFixed(0)}%)</span><span className="tabular-nums">{money(tax)}</span></div>
            <div className="mt-1 flex justify-between border-t border-hairline pt-2 text-[14px] font-semibold text-ink"><span>Total</span><span className="tabular-nums">{money(total)}</span></div>
          </div>
          <CardFooter>
      ...
      <Section title="Install"><CodeBlock language="bash" code={`npx cyanideui add invoice-summary`} /></Section>
    </>
  )
}
