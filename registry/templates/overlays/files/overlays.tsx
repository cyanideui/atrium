 ...
 *
 * State lives in the page so the overlays reflect real data. Swap SEED for
 * your data source and replace the save/delete handlers with API calls.
 */
...
// @atrium:endif
  ...
  const activeFilters = (statusFilter !== "all" ? 1 : 0) + (minAmount ? 1 : 0)
  ...
  function saveForm() {
    ...
    if (editingId) {
      ...
      setOrders((prev) => [
        { id, customer: draft.customer || "Untitled", amount, status: draft.status },
        ...prev,
      ])
            ...
            <div className="flex items-center gap-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="secondary" size="sm" leading={<Icon icon={FilterIcon} size="sm" />}>
                    Filter
                    {activeFilters > 0 && (
          ...
          <TableBody>
            {visible.map((order) => (
              <TableRow key={order.id}>
                ...
                <TableCell>
                  <Badge tone={STATUS_TONE[order.status]}>{titleCase(order.status)}</Badge>
                </TableCell>
