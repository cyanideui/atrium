...
- [ ] **Reduced motion** — under `prefers-reduced-motion: reduce`, all animation/transition collapses to `0ms` (true instant cut, no perceptible motion); looping utilities hard-stopped.
...
The goal is **no perceptible motion at all** — not a faster animation. Every transition and one-shot enter animation (dropdown/select/popover/modal/drawer fade-zoom-slide, sliding pills, knob bounces) collapses to `0ms` — a true instant cut. We set `animation-duration`/`transition-duration` to `0` (not `animation: none`) on purpose: Radix primitives gate their exit-content unmount on the `animationend` event, and a `0s`-duration animation still fires that event (the `animate-out` utilities keep a non-`none` `animation-name`, so Radix waits, gets the event instantly, and unmounts cleanly). Using `animation: none` here would risk exit content that never unmounts. Looping utilities are separately hard-stopped with `animation: none` so they can't freeze mid-cycle. Skeleton shimmer becomes a static block; spinners stop and show as a 50%-opacity dotted ring.
...
- **Reduced motion:** `prefers-reduced-motion: reduce` collapses **every** transition and one-shot enter animation to `0ms` — a true instant cut, **no perceptible motion at all** (no dropdown/select/popover/modal/drawer fade-zoom-slide, no transforms, no shimmer, no pulse, no slide, no scale). We zero the *duration* (not `animation: none`) deliberately: Radix gates exit-content unmount on the `animationend` event, which still fires for a `0s` animation (the `animate-out` utilities keep a non-`none` `animation-name`), so overlays unmount cleanly instead of getting stuck mounted. The global rule also **hard-stops looping utilities** (`.animate-{spin,ping,pulse,bounce}`, skeleton shimmer/pulse) with `animation: none` so they can't freeze mid-cycle — backed by per-component guards: `<Spinner>` → `motion-reduce:animate-none` + 50% dotted ring, `<AutoSaveStatus>` saving icon → `motion-reduce:animate-none`, `<WorkflowTimeline>` active-dot ping → `motion-reduce:hidden`, `<Skeleton>` → static block, Search Field clear ✕ → hidden. Covered by a guard test suite (`reduced-motion.test.tsx`). Blocks/templates inherit this automatically (they compose these guarded primitives + the global rule).
...
**Added — primitives**
- `<AnimatedNumber>` — count-up tween (`mode="count"`, rAF + ease-out-cubic) or per-digit flip/blur (`mode="pop"`). `leading`/`trailing` affixes, `decimals`, `format`. Tabular-nums for stable width. Reduced motion jumps to the final value (no tween, no digit animation) via `useReducedMotion()`. Wired into `<ImportPreview>` stat tiles. *(v1.4.0: added `animateOnMount` — count up from 0 on mount; combine with a changing `key` to replay on demand.)*
...
- **transitions.dev parity:** 17 of the 18 transitions.dev effects ship as real implementations (see the audit table). The 18th — **page forward/back** — is a routing concern, so instead of a primitive the Motion page documents a **copy-paste recipe** using the native View Transitions API + Atrium's `--dur-*`/`--ease-*` tokens (React Router `startViewTransition` wrap + Next.js `experimental.viewTransition`). It respects `prefers-reduced-motion` automatically and degrades to an instant cut where unsupported.
...
**Changed**
- The global `prefers-reduced-motion: reduce` rule in `globals.css` was strengthened from an **80ms cap** to a **full `0ms` hard-cut** (initially landed as `0.01ms`, then tightened to a true `0ms` on review). The 80ms cap still played a brief, visible fade/zoom/slide on every overlay (dropdown menu, select, popover, modal, drawer, tooltip) — the user reported "when I click a dropdown there's still unnecessary animation." Now: `animation-duration`/`transition-duration` collapse to `0ms` with `animation-delay`/`transition-delay` zeroed, so motion-disabled machines get a **true instant cut with no perceptible motion**.
- We zero the *duration* (not `animation: none`) for transitions/one-shot animations on purpose: Radix-based primitives (Dropdown, Select, Popover, Modal, Drawer) gate their exit-content unmount on the `animationend` event. A `0s`-duration animation still fires that event (the `animate-out` utilities keep a non-`none` `animation-name`, so Radix waits, gets the event instantly, and unmounts cleanly) — using `animation: none` here would risk exit content that never unmounts. The looping utilities (`.animate-{spin,ping,pulse,bounce}`, skeleton shimmer/pulse) keep their separate explicit `animation: none` hard-stop.
- No component-file changes needed — the fix is one global rule, and both apps (`example-next`, `playground`) plus the registry's copy-paste consumers `@import "@cyanideui/ui/styles/globals.css"`, so they inherit it automatically. Library tests stay at 64 passing.
